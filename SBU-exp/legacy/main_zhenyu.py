...
class AdversarialTraining(object):
    ...
    '''
    def create_architecture_adversarial(self, batch_size, scope, videos, utility_labels, budget_labels, dropout, is_training, lambda_):
        degrad_videos = residualNet(videos, is_video=True)
    ...
    def run_pretraining_utility(self):
        if not os.path.exists(FLAGS.deg_target_models):
            os.makedirs(FLAGS.deg_target_models)
        ...
        with graph.as_default():
            ...
            if use_pretrained_model:
                ...
                saver = tf.train.Saver(tf.trainable_variables())
                ckpt = tf.train.get_checkpoint_state(checkpoint_dir=FLAGS.deg_target_models)
                if ckpt and ckpt.model_checkpoint_path:
                    saver.restore(self.sess, ckpt.model_checkpoint_path)
                    print('Session restored from trained model at {}!'.format(ckpt.model_checkpoint_path))
                else:
                    raise FileNotFoundError(errno.ENOENT, os.strerror(errno.ENOENT), FLAGS.deg_target_models)
            ...
            for step in xrange(500):
                ...
                if step % self.cfg['TRAIN']['SAVE_STEP'] == 0 or (step + 1) == self.cfg['TRAIN']['MAX_STEPS']:
                    checkpoint_path = os.path.join(FLAGS.deg_target_models, 'model.ckpt')
                    saver.save(self.sess, checkpoint_path, global_step=step)
    ...
    def run_adversarial_training(self):
                ...
                for gpu_index in range(0, self.cfg['TRAIN']['GPU_NUM']):
                    with tf.device('/gpu:%d' % gpu_index):
                        ...
                        with tf.name_scope('%s_%d' % ('gpu', gpu_index)) as scope:
                            ...
                            loss_degrad, loss_budget, loss_utility, logits_budget, logits_utility, argmax_adverse_budget_loss = self.create_architecture_adversarial(scope, self.cfg['TRAIN']['BATCH_SIZE'],videos, utility_labels, budget_labels, dropout_placeholder, is_training_placeholder, _lambda_op)
        ...
        config.gpu_options.allow_growth = True
        # self.sess = tf.Session(graph=graph, config=config)
        with tf.Session(graph=graph, config=config) as self.sess:
            coord = tf.train.Coordinator()
            threads = tf.train.start_queue_runners(sess=self.sess, coord=coord)
            init_op = tf.group(tf.local_variables_initializer(), tf.global_variables_initializer())
            self.sess.run(init_op)
            ...
            if use_pretrained_model:
                ...
                restore_model_ckpt(self.sess, FLAGS.deg_target_models, vardict, "DegradationModule")
            else:
                saver = tf.train.Saver(tf.trainable_variables())
                ...
                for gpu_index in range(0, self.cfg['TEST']['GPU_NUM']):
                    with tf.device('/gpu:%d' % gpu_index):
                        ...
                        with tf.name_scope('%s_%d' % ('gpu', gpu_index)) as scope:
                            ...
                            _, _, _, logits_budget, logits_utility, _ = self.create_architecture_adversarial(scope, self.cfg['TEST']['BATCH_SIZE'], videos, utility_labels, budget_labels, dropout_placeholder, is_training_placeholder, None)
                            logits_budget_lst.append(logits_budget)
                            logits_utility_lst.append(logits_utility)
                            tf.get_variable_scope().reuse_variables()
