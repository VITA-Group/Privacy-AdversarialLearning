...
def tower_loss_xentropy_sparse(logits, labels, use_weight_decay=False, name_scope=None):
    labels = tf.cast(labels, tf.int64)
    xentropy_mean = tf.reduce_mean(
        tf.nn.sparse_softmax_cross_entropy_with_logits(logits=logits, labels=labels))

    if use_weight_decay:
        tf.add_to_collection('c3d_losses', xentropy_mean) # l2 regularization is already in 'c3d_losses' namespace. (Added in utilityNet.py)
