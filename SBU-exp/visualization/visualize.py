...
sys.path.insert(0, '..')
from utils import avg_replicate
        ...
        for gpu_index in range(0, cfg['GPU_NUM']):
            with tf.device('/gpu:%d' % gpu_index):
                ...
                with tf.name_scope('%s_%d' % ('gpu', gpu_index)) as scope:
                    videos_degraded = residualNet(videos_placeholder[gpu_index * cfg['BATCH_SIZE']:(gpu_index + 1) * cfg['BATCH_SIZE']], is_video=True)
                    videos_degraded = avg_replicate(videos_degraded)
...
def main(_):
    checkpoint_dir = '../adversarial_training/ckpt_dir/AlternativeUpdate-NoRest-M1/'
