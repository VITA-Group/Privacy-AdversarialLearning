...
def write_video(videos, labels, width=160, height=120, nchannel=3):
    class_dict = {}
    with open('privacy_split/classInd.txt', 'r') as f:
        for line in f:
            print(line)
            words = line.strip('\n').split()
...
def memoryhog(folder):
    directory = 'data_tfrecords'
    print(os.path.join('hmdb51_shuffled', folder))
    X_train, Y_train, X_test, Y_test = video_processing(videos_dir=os.path.join('hmdb51_shuffled', folder))
...
if __name__ == "__main__":
    ...
    num_cores = 20
    if not os.path.exists('data_tfrecords'):
        os.makedirs('data_tfrecords')
    #memoryhog('wrong_videos')
    Parallel(n_jobs=num_cores)(delayed(memoryhog)(q) for q in next(os.walk('hmdb51_shuffled'))[1])
